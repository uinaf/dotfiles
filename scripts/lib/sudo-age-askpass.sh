...
if [ -n "${SUDO_AGE_BIN:-}" ] \
  && [ -n "${SUDO_AGE_IDENTITY_FILE:-}" ] \
  && [ -n "${SUDO_AGE_CIPHERTEXT_FILE:-}" ]; then
  age_bin="$SUDO_AGE_BIN"
  identity_file="$SUDO_AGE_IDENTITY_FILE"
  ciphertext_file="$SUDO_AGE_CIPHERTEXT_FILE"
else
...
exec "$age_bin" --decrypt -i "$identity_file" "$ciphertext_file"
