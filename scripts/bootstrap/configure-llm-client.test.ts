...
import { assertCursorAgentBinSafe, gatewayEdits, parseClientConfig } from "./configure-llm-client.ts";
...
`, { mode: 0o700 });
    ...
    const state = JSON.parse(readFileSync(join(configDir, "llm-client-state.json"), "utf8")) as { version: number; cursorCommands: Array<{ target: string }> };
