...
for required in 'brew "awscli"' 'brew "docker-credential-helper"' 'brew "glab"' 'brew "opencode"' 'brew "xcodegen"' 'cask "android-commandlinetools"'; do
  grep -Fqx "$required" "$repo_root/Brewfile.developer" \
    || fail "developer layer missed $required"
...
for expected in 'node = { version = "24.19.0"' 'npm@12.0.2' 'pnpm@11.23.0' 'bun = "1.4.0"' 'java = "temurin-21"' 'ruby = "4.0.6"' 'go = "1.26.6"' '"npm:@playwright/cli" = "0.1.18"' 'trusted_config_paths'; do
