...
for (const failure of [false, true]) {
  test(`maintenance reuses profile setup with saved logins preserved${failure ? " and stops on failure" : ""}`, t => {
    ...
    for (const name of ["apply-dotfiles.ts", "install-oh-my-zsh.ts", "install-cursor-agent.ts", "trust-agent-worktrees.ts", "install-gh-extensions.ts", "configure-codex.ts", "configure-llm-gateway.ts", "configure-bifrost-clients.ts"]) stub(join(root, "scripts/bootstrap", name), name);
