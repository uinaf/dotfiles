...
for (const scenario of ["success", "failure", "spawn", "delivery", "invalid", "unconfigured"] as const) {
  test(`update reporting preserves ${scenario} without repeating the command`, async t => {
    ...
    const send: typeof fetch = async (input, init) => {
      ...
      return new Response(null, { status: scenario === "delivery" ? 503 : 200 });
    };
    const result = await Effect.runPromise(runUpdate("software-update", home, "fixture", [], send).pipe(
    ...
    assert.deepEqual(sent, scenario === "invalid" || scenario === "unconfigured" ? [] : [`${url}${expected ? "/fail" : ""}`]);
  });
}
