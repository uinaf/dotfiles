...
const program = Effect.gen(function* () {
  ...
  if ((yield* fs.exists(agentPath)) && !process.env.CURSOR_AGENT_INSTALLER_URL) {
    ...
    if (updated.status !== 0)
      return yield* fail(`Cursor Agent update exited ${updated.status}`, updated.status);
  } else {
    yield* Console.log("downloading the official Cursor Agent installer");
    const download = yield* runner.run("curl", ["-fsSL", installerUrl, "-o", installerPath], {
      output: "inherit",
    });
    if (download.status !== 0)
      return yield* fail(`Cursor Agent download exited ${download.status}`);
