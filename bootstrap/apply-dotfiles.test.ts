...
import { convergeUserManager, pruneOlderBackups, retireLaunchAgents, retiredAgentLabels } from "./apply-dotfiles.ts";
...
for (const unavailable of ["exit", "spawn"] as const) {
  test(`apply tolerates an unavailable user manager (${unavailable})`, async () => {
    ...
    await Effect.runPromise(convergeUserManager("/home/test", false, "linux").pipe(Effect.provideService(CommandRunner, runner)));
...
test("apply converges manager PATH idempotently without losing unrelated entries", async () => {
  ...
  const runner = CommandRunner.of({ run: (_command, args = []) => {
    ...
    return Effect.succeed({ status: 0, stdout: args[1] === "show-environment" ? `OTHER=value\nPATH=${path}\n` : "", stderr: "" });
  } });
  for (let index = 0; index < 2; index++) {
    await Effect.runPromise(convergeUserManager(home, false, "linux").pipe(Effect.provideService(CommandRunner, runner)));
    assert.equal(path, [...front, "/custom/bin", "/usr/bin"].join(":"));
  }
  assert.deepEqual(calls.map(call => call[1]), ["show-environment", "daemon-reload", "set-environment", "show-environment", "daemon-reload", "set-environment"]);
});
...
for (const operation of ["daemon-reload", "set-environment"]) {
  for (const failure of ["exit", "spawn"]) {
    test(`reachable manager ${operation} ${failure} failure is reported`, async () => {
      const runner = CommandRunner.of({ run: (command, args = []) => args[1] === operation
        ? failure === "spawn" ? Effect.fail(new CommandError({ command, message: "unavailable" })) : Effect.succeed({ status: 7, stdout: "", stderr: "denied" })
        : Effect.succeed({ status: 0, stdout: "PATH=/usr/bin", stderr: "" }) });
      const error = await Effect.runPromise(convergeUserManager("/home/test", false, "linux").pipe(Effect.provideService(CommandRunner, runner), Effect.flip));
...
test("dry-run and non-Linux apply never contact systemd", async () => {
  ...
  await Effect.runPromise(convergeUserManager("/home/test", true, "linux").pipe(Effect.provideService(CommandRunner, runner)));
  await Effect.runPromise(convergeUserManager("/home/test", false, "darwin").pipe(Effect.provideService(CommandRunner, runner)));
