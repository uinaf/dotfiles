...
const installerUrl =
  process.env.TIZEN_INSTALLER_URL ||
  `https://download.tizen.org/sdk/Installer/tizen-sdk_${version}/${installerName}`;
...
const usage = `usage: bootstrap/darwin/tizen/install.ts [--show-pkgs] [--packages package1,package2]
...
const parseArguments = Effect.fn("parseTizenInstallArguments")(function* (
...
const downloadInstaller = Effect.fn("downloadTizenInstaller")(function* (
  javaEnv: Readonly<Record<string, string>>,
) {
  ...
  if (requested === "aria2c" || (yield* commandAvailable("aria2c"))) {
    if (!(yield* commandAvailable("aria2c")))
      return yield* fail("missing required command: aria2c");
...
const program = Effect.gen(function* () {
  ...
  if (process.platform !== "darwin")
    return yield* fail("this installer script is currently macOS-only");
  ...
  if (!(yield* executable(packageManager))) {
    if (yield* fs.exists(studioHome)) {
      return yield* fail(
        `Tizen Studio path exists but package manager is missing: ${studioHome}\nmove or remove that directory, then rerun this script`,
      );
    }
  ...
  if (!(yield* executable(packageManager)))
    return yield* fail(`package manager not found after install: ${packageManager}`);
  ...
  if (args.packages) {
    ...
    yield* run(
      packageManager,
      ["install", "--accept-license", "--no-java-check", "--proxy", packageProxy, args.packages],
      javaEnv,
    );
  }
  ...
  if (args.showPackages) {
    yield* Console.log("\nAvailable packages:");
    yield* Console.error(
      "warning: Samsung package catalog lookup can be slow or hang on extension downloads",
    );
    yield* run(packageManager, ["show-pkgs", "--proxy", packageProxy, "--tree"], javaEnv);
  }
  yield* Console.log("\nTizen SDK install step finished.");
  yield* Console.log(
    "Package catalog lookup is intentionally skipped by default; use --show-pkgs only when needed.",
  );
  yield* Console.log(
    "Next, restore cert/profile state if needed:\n  ./bootstrap/darwin/tizen/restore.ts ~/Desktop/tizen-certs-YYYYMMDDHHMMSS.tar.gz",
  );
