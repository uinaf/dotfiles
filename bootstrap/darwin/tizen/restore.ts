...
const runTar = Effect.fn("runTizenRestoreTar")(function* (home: string, archive: string) {
  ...
  const result = yield* runner
    .run("tar", ["-C", home, "-xzf", archive], { stdin: "inherit", output: "inherit" })
...
const program = Effect.gen(function* () {
  ...
  if (args.length !== 1)
    return yield* fail(
      "usage: bootstrap/darwin/tizen/restore.ts /path/to/tizen-migration.tar.gz",
      2,
    );
  ...
  if (Option.isNone(info) || info.value.type !== "File")
    return yield* fail(`archive not found: ${archive}`);
  ...
  for (const [source, name] of [
    ...
    if (
      Option.isSome(sourceInfo) &&
      sourceInfo.value.type === "File" &&
      (sourceInfo.value.mode & 0o111) !== 0
    ) {
