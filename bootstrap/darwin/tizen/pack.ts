...
const runTar = Effect.fn("runTizenPackTar")(function* (
  home: string,
  output: string,
  ...
  const result = yield* runner
    .run("tar", ["-C", home, "-czf", output, ...paths], { output: "inherit" })
...
const program = Effect.gen(function* () {
  ...
  if (args.length > 1 || args[0] === "--full")
    return yield* fail("usage: bootstrap/darwin/tizen/pack.ts [--full] [OUTPUT]", 2);
  ...
  const output =
    args[0] || join(home, "Desktop", `tizen-${full ? "migration" : "certs"}-${timestamp}.tar.gz`);
