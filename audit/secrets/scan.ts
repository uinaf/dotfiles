...
import { type PathSource, resolveSources } from "../paths.ts";
...
export function scanSecrets(
  ...
  const paths = resolveSources(home, sources);
  ...
  try {
    ...
    const gitleaks = command("gitleaks", [
      "dir",
      "--follow-symlinks",
      "--redact",
      "--exit-code",
    ...
    const trufflehog = command("trufflehog", [
      "filesystem",
      "--no-update",
      "--no-color",
      "--results=verified",
