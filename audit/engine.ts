...
function resolveSources(home: string, sources: readonly PathSource[]): string[] {
...
function checkSecretScan(run: AuditRun, sources: readonly PathSource[]): void {
  ...
  if (paths.length === 0)
    return run.finding("warn", "no readable local config files found for gitleaks secret scan");
  ...
  try {
    ...
    const gitleaks = run.command("gitleaks", [
      "dir",
      "--follow-symlinks",
      "--redact",
      "--exit-code",
    ...
    const summary = summarizeFindings(
      JSON.stringify(run.secret.rules),
      JSON.stringify(run.secret.severities),
      report,
    ...
    if (summary.findingCount === 0 && gitleaks.status === 0)
      run.finding("ok", `gitleaks found no leaks in ${paths.length} local config files`);
    ...
    const trufflehog = run.command("trufflehog", [
      "filesystem",
      "--no-update",
      "--no-color",
      "--results=verified",
    ...
    if (trufflehog.status === 0)
      run.finding("ok", `trufflehog found no verified leaks in ${paths.length} local config files`);
    else if (trufflehog.status === 183)
      run.finding("fail", "trufflehog reported verified leaks in local config files");
...
function checkLogFile(run: AuditRun, path: string): void {
  ...
  if (live >= warnBytes)
    run.finding("warn", `${path} live data is larger than ${humanBytes(warnBytes)} (${detail})`);
...
function runCheck(run: AuditRun, check: AuditCheck): void {
  switch (check.kind) {
    ...
    case "secret-scan":
      return checkSecretScan(run, check.sources);
    case "private-mode":
...
export function runPolicy(
  ...
  for (const key of [
    "CODEX_LOG_FAIL_BYTES",
    "CODEX_LOG_WARN_BYTES",
    "CODEX_LOG_RECLAIM_WARN_BYTES",
    "CODEX_LOG_FREELIST_WARN_RATIO",
    "CODEX_LOG_RECLAIM_FLOOR_BYTES",
  ]) {
