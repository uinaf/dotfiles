...
export async function hygiene(home: string, apply: boolean, scheduled: boolean, now = Date.now(), signal?: AbortSignal): Promise<void> {
  ...
  try {
    ...
    const cache = cacheDue
      ? await Effect.runPromise(cacheCleanup(home, apply).pipe(Effect.provide(CommandRunner.layer), Effect.provide(NodeServices.layer)), { signal })
...
if (process.argv[1] && fileURLToPath(import.meta.url) === resolve(process.argv[1])) {
  ...
  try {
    ...
    await hygiene(homedir(), process.argv.includes("--apply") && !process.argv.includes("--dry-run"),
      process.argv.includes("--scheduled") && !process.argv.includes("--dry-run"), Date.now(), controller.signal);
  } catch (error) {
    if (!interrupted) {
