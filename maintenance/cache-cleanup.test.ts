...
async function fixture(t: test.TestContext) {
  ...
  for (const name of ["xcrun", "pnpm", "docker"]) await writeFile(join(bin, name), "fixture", { mode: 0o755 });
  ...
  const calls: string[][] = [];
  const run = (apply: boolean, options: { failure?: string; spawnFailure?: boolean; probeFailure?: boolean } = {}) => Effect.runPromise(
    Effect.gen(function*() {
      const live = yield* CommandRunner;
      const runner = CommandRunner.of({ run: (command, args = [], commandOptions) => {
        assert.equal(commandOptions?.cwd, home);
        assert.equal(commandOptions?.env?.HOME, home);
        calls.push([command, ...args]);
        if (command === options.failure) return options.spawnFailure
        ...
        return Effect.succeed({ status: options.probeFailure && (args[0] === "info" || args[0] === "--find") ? 1 : 0,
          stdout: command === "df" ? "Filesystem 1024-blocks Used Available Capacity Mounted\nfixture 2000000 1000000 1000000 50% /\n" : "", stderr: "" });
      } });
...
test("cache ages match find day boundaries, dry-run preserves everything, and apply keeps exclusions", async t => {
  ...
  await Promise.all([old, oldLog, boundary, recent, boundaryLog, ...exclusions].map(path => access(path)));
  assert.ok(f.calls.every(call => !call.includes("-delete") && !call.includes("prune") && !call.includes("delete")));
  ...
  assert.ok(f.calls.some(call => call.join(" ") === "docker builder prune -f --filter until=168h"));
...
test("optional tool probes gate cleanup and missing tools are skipped", async t => {
  ...
  assert.ok(f.calls.every(call => call[0] !== "pnpm" && !call.includes("delete") && !call.includes("prune")));
});
...
test("one total budget cancels the active command and prevents subsequent cleanup", async t => {
  ...
  const runner = CommandRunner.of({ run: command => {
    calls.push(command);
    return command === "df"
      ? Effect.sleep(10).pipe(Effect.as({ status: 0, stdout: "header\nfixture 2000000 1000000\n", stderr: "" }))
      : Effect.never.pipe(Effect.ensuring(Effect.sync(() => { cancelled = true; })));
  } });
  const result = await Effect.runPromise(cacheCleanup(f.home, true, 40).pipe(
