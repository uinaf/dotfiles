...
export const manageSchedule = Effect.fn("manageSchedule")(function*(action: string, home: string, uid: number) {
  ...
  const linger = Effect.gen(function*() {
    const result = yield* runner.run("loginctl", ["show-user", String(uid), "--property=Linger", "--value"], { output: "capture" });
  ...
  switch (action) {
    ...
    case "disable": {
      ...
      for (const args of [["disable", "--now", `${updateUnit}.timer`], ["stop", `${updateUnit}.service`]]) {
        const result = yield* systemctl(...args).pipe(Effect.catch((error) => Effect.succeed({ status: 1, stdout: "", stderr: error.message })));
...
if (process.argv[1] && fileURLToPath(import.meta.url) === resolve(process.argv[1])) {
  const program = process.argv.length === 3
