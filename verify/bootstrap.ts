...
import { brewfilePath, bundleCheckArgs, bundleDrift, configureExternalCapabilities, profileBrewfiles, runHomebrewRaw, verifyPrefixPermissions, withLocalBrewfile } from "../homebrew/homebrew.ts";
...
const program = Effect.gen(function*() {
  ...
  const environment = Effect.gen(function*() {
    ...
    const trust = yield* runner.run(process.execPath, [join(repoRoot, "bootstrap/trust-agent-worktrees.ts"), "--check"]);
  ...
  const profileTools = Effect.gen(function*() {
    ...
    if (darwin) yield* command(process.execPath, [join(repoRoot, "bootstrap/darwin/xcode.ts"), "--check"]);
    if (config.capabilities.devbox) {
      yield* shellChecks(["tmux -V", "tailscale status --peers=false", ...(darwin ? ["xcodes version"] : [])]);
      yield* command(process.execPath, [join(repoRoot, "bootstrap/install-t3-service.ts"), "--check"]);
    }
  });
  ...
  const configuration = Effect.gen(function*() {
    ...
    if (config.capabilities.requiresSopsIdentity) yield* command(process.execPath, [join(repoRoot, "identity/configure-sops-age-identity.ts"), "--check"]);
  ...
  const host = Effect.gen(function*() {
    if (!darwin) return;
    yield* command(process.execPath, [join(repoRoot, "bootstrap/darwin/configure-spotlight.ts"), "--check"]);
    if (desktop) yield* command(process.execPath, [join(repoRoot, "bootstrap/darwin/configure-desktop.ts"), "--check"]);
  });
