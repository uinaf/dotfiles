...
const program = Effect.scoped(Effect.gen(function*() {
  ...
  assert.deepEqual(profileBrewfiles(model, "devbox"), ["homebrew/Brewfile", "homebrew/Brewfile.devbox"]);
  assert.deepEqual(profileBrewfiles(model, "personal-devbox"), ["homebrew/Brewfile", "homebrew/Brewfile.devbox", "homebrew/Brewfile.personal"]);
  assert.deepEqual(profileBrewfiles(model, "workstation"), ["homebrew/Brewfile", "homebrew/Brewfile.workstation"]);
  assert.deepEqual(profileBrewfiles(model, "personal-workstation"), ["homebrew/Brewfile", "homebrew/Brewfile.workstation", "homebrew/Brewfile.personal", "homebrew/Brewfile.personal-workstation"]);
  for (const profile of ["personal-devbox", "devbox"]) {
    assert.deepEqual(bundleCheckArgs(model, profile, "homebrew/Brewfile"), ["bundle", "check", "--no-upgrade", "--file", "homebrew/Brewfile"]);
  }
  for (const profile of ["personal-workstation", "workstation"]) {
    assert.deepEqual(bundleCheckArgs(model, profile, "homebrew/Brewfile"), ["bundle", "check", "--file", "homebrew/Brewfile"]);
  }
  ...
  const resolved = yield* run(process.execPath, [join(repoRoot, "agents/resolve-profile.ts"), "--expected", "personal-devbox"], { env: { HOME: agentHome } });
  assert.equal(resolved.status, 0, resolved.stderr);
  assert.equal(resolved.stdout.trim(), "personal-devbox");
  assert.equal((yield* run(process.execPath, [join(repoRoot, "agents/resolve-profile.ts"), "--expected", "devbox"], { env: { HOME: agentHome } })).status, 3);
  ...
  assert.notEqual((yield* run(process.execPath, [join(repoRoot, "bootstrap/apply-dotfiles.ts"), "--profile", "devbox"], { env: { HOME: canonicalSymlinkHome } })).status, 0);
  ...
  const developerSteps = (yield* run(process.execPath, [join(repoRoot, "bootstrap/install.ts"), "--print-steps", "--profile", "developer"])).stdout.trim().split("\n");
