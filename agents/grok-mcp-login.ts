...
const USAGE = `Usage: ./agents/grok-mcp-login.ts [--no-browser] [--port PORT] SERVER
  ...
  for (let index = 0; index < args.length; index += 1) {
    ...
    if (arg === "--no-browser") {
      ...
      if (!Number.isInteger(value) || value <= 0 || value > 65535)
        return { error: "--port requires a TCP port" };
      port = value;
      index += 1;
...
export function serverUrlFromConfig(toml: string, server: string): string | undefined {
  ...
  const header = new RegExp(
    `^\\[mcp_servers\\.(?:"${server}"|${server.replace(/[.*+?^${}()|[\]\\]/g, "\\$&")})\\]\\s*$`,
  );
...
async function fetchJson(url: string, init?: RequestInit): Promise<Record<string, unknown>> {
  ...
  if (!response.ok)
    throw new Error(`${url} returned ${response.status}: ${JSON.stringify(body).slice(0, 200)}`);
...
async function discover(serverUrl: string): Promise<Metadata> {
  ...
  const resource: Record<string, unknown> = await fetchJson(
...
async function login(options: Options): Promise<void> {
  ...
  const code = await new Promise<string>((resolvePromise, rejectPromise) => {
    const server = createServer((request, response) => {
      ...
      response
        ...
        .end("Grok MCP login complete. You can close this tab.");
      server.close();
      clearTimeout(timer);
      ...
      if (value) resolvePromise(value);
        ...
        rejectPromise(
          new Error(url.searchParams.get("error_description") ?? "no authorization code"),
        );
    });
  ...
  process.stdout.write(
    `Stored Grok MCP credentials for ${options.server}. Verify with: grok mcp doctor\n`,
  );
}
...
const program = Effect.gen(function* () {
  ...
  yield* Effect.tryPromise({
    try: () => login(parsed),
    catch: (error) => new Error(`Grok MCP login failed: ${errorMessage(error)}`),
  });
});
