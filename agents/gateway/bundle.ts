...
export function gatewayInterpreter(executable: string): string {
  ...
  const stable = executable.replace(
    /\/installs\/node\/[^/]+\/bin\/node$/,
    "/installs/node/latest/bin/node",
  );
  return existsSync(stable) ? stable : executable;
}

export async function bundleGatewayHelpers(
  executable = process.execPath,
): Promise<Record<GatewayHelper, string>> {
  const interpreter = gatewayInterpreter(executable);
  if (/[\r\n]/.test(interpreter))
    throw new Error("gateway interpreter path must not contain line breaks");
  ...
  const result = await build({
    absWorkingDir: root,
    entryPoints: gatewayHelpers.map((name) => `agents/gateway/${name}.ts`),
    outdir: "gateway-helpers",
    bundle: true,
    write: false,
    platform: "node",
    format: "cjs",
  ...
  const helper = (name: GatewayHelper): string => {
    const output = result.outputFiles.find(
      (file) => file.path === join(root, "gateway-helpers", `${name}.js`),
    );
