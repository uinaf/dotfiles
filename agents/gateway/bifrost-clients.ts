...
const BifrostKey = Schema.String.pipe(
  Schema.check(
...
const parseJsonObject = Effect.fn("parseJsonObject")(function* (source: string, label: string) {
  ...
  return yield* Schema.decodeUnknownEffect(JsonObject)(parsed).pipe(
    Effect.mapError(
      () => new CliFailure({ exitCode: 1, message: `${label} must contain a JSON object` }),
    ),
  );
});
...
const readObject = Effect.fn("readObject")(function* (path: string, label: string) {
  ...
  if (Option.isSome(link) || info.value.type !== "File")
    return yield* fail(`${label} must be a regular file: ${path}`);
  return { info, value: yield* parseJsonObject(yield* fs.readFileString(path), label) };
});

const writeObject = Effect.fn("writeObject")(function* (
  path: string,
  filename: string,
  value: unknown,
) {
...
export const configureBifrostClients = Effect.fn("configureBifrostClients")(function* (options: {
  ...
  if (Option.isSome(helperLink) || Option.isNone(helperInfo) || helperInfo.value.type !== "File")
    return yield* fail(`credential helper must be a regular file: ${helper}`);
  if ((helperInfo.value.mode & 0o111) === 0)
    return yield* fail("credential helper must be executable");
  const gateway = yield* readObject(gatewayPath, "gateway config");
  if (Option.isNone(gateway.info) || (gateway.info.value.mode & 0o077) !== 0)
    return yield* fail("gateway config must be owner-only");
  const decodedGateway = yield* Schema.decodeUnknownEffect(GatewayConfig)(gateway.value).pipe(
    Effect.mapError(
      () => new CliFailure({ exitCode: 1, message: "gateway config must contain bifrostBaseUrl" }),
    ),
  );
  const bifrostUrl = yield* Effect.try({
    try: () => new URL(decodedGateway.bifrostBaseUrl),
    catch: () =>
      new CliFailure({ exitCode: 1, message: "bifrostBaseUrl must be an HTTPS /v1 URL" }),
  });
  if (
    bifrostUrl.protocol !== "https:" ||
    bifrostUrl.username ||
    bifrostUrl.password ||
    bifrostUrl.search ||
    bifrostUrl.hash ||
    !bifrostUrl.pathname.endsWith("/v1")
  ) {
    return yield* fail("bifrostBaseUrl must be an HTTPS /v1 URL");
  }
  const runner = yield* CommandRunner;
  const credential = yield* runner
    .run(helper, ["bifrost"])
    .pipe(
      Effect.mapError(
        () => new CliFailure({ exitCode: 1, message: "Bifrost credential helper failed" }),
      ),
    );
  ...
  const key = yield* Schema.decodeUnknownEffect(BifrostKey)(credential.stdout.trim()).pipe(
    Effect.mapError(
      () =>
        new CliFailure({
          exitCode: 1,
          message: "credential helper returned an invalid Bifrost key",
        }),
    ),
  );
  ...
  const openCodeProviders =
    openCode.value.provider === undefined
      ? {}
      : yield* Schema.decodeUnknownEffect(JsonObject)(openCode.value.provider).pipe(
          Effect.mapError(
            () =>
              new CliFailure({
                exitCode: 1,
  ...
  const onlyBifrostEnabled =
    Array.isArray(openCode.value.enabled_providers) &&
    openCode.value.enabled_providers.length === 1 &&
    openCode.value.enabled_providers[0] === "bifrost";
  if (check) {
    if (
    ...
    if (
      Option.isNone(openCode.info) ||
      (openCode.info.value.mode & 0o077) !== 0 ||
      !onlyBifrostEnabled ||
