...
before(async () => { bundles = await bundleGatewayHelpers(); });
...
function fixture(t: TestContext, installedBundles = bundles) {
  ...
  mkdirSync(bin);
  for (const [name, contents] of Object.entries(installedBundles)) writeFileSync(join(installed, name), contents, { mode: 0o700 });
...
`, { mode: 0o700 });
  ...
  const env = { ...process.env, HOME: home, PATH: bin, NODE_PATH: "", LLM_GATEWAY_CONFIG: undefined };
  const run = (name: GatewayHelper, args: string[] = [], extra: NodeJS.ProcessEnv = {}) => spawnSync(join(installed, name), args, {
    cwd: root, encoding: "utf8", env: { ...env, ...extra }, timeout: 10_000,
  });
...
test("installed adapters run outside the checkout with no module graph, jq, Python, or Node on PATH", (t) => {
  ...
  assert.deepEqual(JSON.parse(about.stdout), { cliVersion: "fixture-version", userEmail: "api-key@local" });
  const vendor = f.run("cursor-agent-api", ["chat", "hello"], { VENDOR_EXIT: "37" });
  assert.equal(vendor.status, 37, vendor.stderr);
  assert.deepEqual(JSON.parse(vendor.stdout), { args: ["chat", "hello"], memory: "memory", keyPresent: true });
});
...
test("credential validation is fail closed and never includes rejected payloads in diagnostics", (t) => {
  ...
  writeFileSync(f.configPath, JSON.stringify({ ...f.config, credentials: { ...f.config.credentials, gatewai: rejected } }));
...
test("Codex launcher pins installed credentials across redirected HOME without putting credentials in argv or env", (t) => {
  const f = fixture(t);
  writeFileSync(join(f.bin, "codex"), `#!${process.execPath}
...
`, { mode: 0o700 });
  const result = f.run("codex-gatewai", ["--ignore-user-config", "exec", "fixture"], {
    HOME: join(f.root, "redirected"), CURSOR_API_KEY: undefined, CLIPROXYAPI_CLIENT_API_KEY: undefined,
  });
  assert.equal(result.status, 0, result.stderr);
  const received = JSON.parse(result.stdout) as { args: string[]; config: string; cursor: null; gatewai: null };
  assert.equal(received.config, f.configPath);
  assert.ok(received.args.includes(`model_providers.gatewai.auth.command=${JSON.stringify(join(f.installed, "llm-gateway-credential"))}`));
...
test("installed bundles handle quoted interpreter paths and ancestor ESM packages without consuming stdin", async (t) => {
  ...
  const forwarded = spawnSync(join(f.installed, "cursor-acp-api-key-auth"), [process.execPath, vendor], {
    cwd: f.root, encoding: "utf8", env: f.env, input: request, timeout: 10_000,
  });
