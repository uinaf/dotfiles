...
async function fixture(t: TestContext, childSource: string, args?: string[]) {
  ...
  const closed = new Promise<{ code: number | null; signal: NodeJS.Signals | null }>(
    (resolve, reject) => {
      const timeout = setTimeout(() => {
        proxy.kill("SIGKILL");
        reject(new Error(`ACP did not exit; stderr=${stderr}`));
      }, 8_000);
...
function waitForStderr(child: ChildProcessWithoutNullStreams, text: string): Promise<void> {
  return new Promise((resolve, reject) => {
    ...
    const timeout = setTimeout(() => {
      ...
      reject(new Error(`missing child marker ${text}`));
    }, 4_000);
...
test("ACP intercepts authenticate requests, forwards notifications and malformed lines, and drains on EOF", async (t) => {
  ...
  const forwarded = [
    "{malformed",
    '{"method":"authenticate"}',
    '{"id":11,"method":"session/new"}',
    "[1,2,3]",
  ];
  f.process.stdin.end(
    [
      ...forwarded,
      '{"jsonrpc":"2.0","id":1,"method":"authenticate","params":{"methodId":"cursor_login"}}',
    ].join("\n") + "\n",
  );
  assert.deepEqual(await f.closed, { code: 0, signal: null });
  assert.deepEqual(
    f.stdout().trim().split("\n").sort(),
    [...forwarded, '{"jsonrpc":"2.0","id":1,"result":{}}'].sort(),
  );
});
...
test("ACP drains authentication replies through a slow downstream before completing", async (t) => {
  ...
  const count = 4_000;
  f.process.stdin.end(
    Array.from({ length: count }, (_, id) => JSON.stringify({ id, method: "authenticate" })).join(
      "\n",
    ) + "\n",
  );
  const resume = setTimeout(() => f.process.stdout.resume(), 100);
  t.onTestFinished(() => clearTimeout(resume));
  assert.deepEqual(await f.closed, { code: 0, signal: null });
  const messages = f
    .stdout()
    .trim()
...
test("ACP drains final frames and preserves status when the child closes stdin early", async (t) => {
  ...
  f.process.stdin.write(
    JSON.stringify({ id: 1, method: "session/new", params: { payload: "x".repeat(200_000) } }) +
      "\n",
  );
