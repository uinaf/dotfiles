...
main(
  Effect.gen(function* () {
    ...
    if (
      ...
      dirname(dirname(target.value)) === join(home, ".local/share/cursor-agent/versions") &&
      target.value.endsWith("/cursor-agent") &&
      executable(target.value)
    )
      agent = target.value;
    if (!agent || !executable(agent))
      return yield* failure("Cursor Agent executable is unavailable");
    const args = process.argv.slice(2);
    const command = args[0];
    if (command === "login" || command === "logout")
      return yield* failure(
        "saved-login changes are disabled while the API-key client is enabled; roll back the LLM gateway first",
      );
    if (["-v", "--version", "-h", "--help"].includes(command))
      return yield* replaceProcess(agent, args);
    const env = {
      ...process.env,
      CURSOR_API_KEY: yield* credential("cursor", home),
    ...
    if (command === "status" || command === "whoami" || command === "about") {
      ...
      return yield* print(
        format === "json"
          ? JSON.stringify({ cliVersion: version, userEmail: "api-key@local" })
          : `About Cursor CLI\n\nCLI Version         ${version}\nUser Email          api-key@local\n`,
      );
    }
    if (args.includes("acp"))
      return yield* replaceProcess(
