...
const program = Effect.scoped(
  Effect.gen(function* () {
    ...
    if (!command)
      return yield* failure("usage: cursor-acp-api-key-auth <cursor-agent> [args...]", 2);
    ...
    const input = NodeStream.fromReadable({ evaluate: () => process.stdin }).pipe(
      ...
      Stream.splitLines,
      Stream.mapEffect(
        Effect.fn("filterCursorAuthentication")(function* (line) {
          const message = yield* Effect.try(() => JSON.parse(line) as unknown).pipe(
    ...
    const writer = Stream.fromQueue(replies).pipe(
      Stream.run(
