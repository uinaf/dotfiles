...
| Testable | pass | `scripts/verify/repo.sh` runs shell syntax, ShellCheck, profile rendering and layer contracts, the shared-host Homebrew wrapper contract, isolated Git bootstrap, Actionlint, diff hygiene, entrypoint checks, and secret scans. | Live bootstrap checks require a matching role on macOS. |
